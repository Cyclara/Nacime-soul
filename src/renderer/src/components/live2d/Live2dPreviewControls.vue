...
watch(
  () => live2d.state.window.zoom,
  (value) => {
    if (!saving.value) zoom.value = value
  }
)
watch(
  () => live2d.state.window.offsetX,
  (value) => {
    if (!saving.value) offsetX.value = value
  }
)
watch(
  () => live2d.state.window.offsetY,
  (value) => {
    if (!saving.value) offsetY.value = value
  }
)
watch(
  () => live2d.state.window.alwaysOnTop,
  (value) => {
    if (!saving.value) alwaysOnTop.value = value
  }
)

async function toggleVisible(): Promise<void> {
  await live2d.setVisible(!visible.value)
}
async function resetPlacement(): Promise<void> {
  await live2d.resetWindowPlacement()
}
...
<template>
  <section class="preview-panel" aria-labelledby="preview-controls-title">
    <div id="preview-controls-title" class="preview-panel__title">窗口控制</div>
    ...
    <div v-if="props.orchestrator" class="settings-grid">
      <div class="framing" role="group" aria-label="取景预设">
        ...
        <button type="button" class="framing__preset" @click="applyFraming('upper-body')">
          半身
        </button>
        <button type="button" class="framing__preset" @click="applyFraming('full-body')">
          全身
        </button>
      </div>
      <label class="setting-row">
        <span
          ><strong>角色大小</strong><small>{{ Math.round(zoom * 100) }}%</small></span
        >
        <input
          v-model="zoom"
          type="range"
      ...
      <label class="setting-row">
        <span
          ><strong>左右位置</strong
          ><small>{{
            offsetX > 0 ? `右 ${offsetX}` : offsetX < 0 ? `左 ${-offsetX}` : '居中'
          }}</small></span
        >
        <input
          v-model="offsetX"
      ...
      <label class="setting-row">
        <span
          ><strong>上下位置</strong
          ><small>{{
            offsetY > 0 ? `上移 ${offsetY}` : offsetY < 0 ? `下移 ${-offsetY}` : '贴底'
          }}</small></span
        >
        <input
          v-model="offsetY"
...
.control--quiet {
  border-color: var(--color-border, rgb(255 255 255 / 14%));
...
.control-hint--ready {
  color: rgb(133 226 174 / 82%);
}
.settings-grid {
  display: grid;
  gap: 0.7rem;
  padding-top: 0.45rem;
  border-top: 1px solid var(--color-border-subtle, rgb(255 255 255 / 8%));
}
.framing {
  display: flex;
  align-items: center;
...
.framing__label {
  flex: 1;
  color: var(--color-text-secondary, rgb(255 255 255 / 72%));
  font-size: 0.75rem;
  font-weight: 600;
}
.framing__preset {
...
.setting-toggle small {
  display: block;
  color: var(--color-text-muted, rgb(255 255 255 / 46%));
  font-size: 0.68rem;
}
...
.save-error {
  margin: 0;
  color: var(--color-error, rgb(255 139 139));
  font-size: 0.72rem;
}
</style>
