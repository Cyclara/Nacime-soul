...
export type StartupStage =
  'checking-data' | 'preparing-memory' | 'waking-nacime' | 'ready' | 'blocked'
const props = withDefaults(defineProps<{ stage: StartupStage; errorMessage?: string }>(), {
  errorMessage: ''
})
const text = computed(
  () =>
...
<template>
  <Transition name="startup-fade" appear>
    <section
      v-if="stage !== 'ready'"
      class="startup"
      :class="{ 'startup--blocked': stage === 'blocked' }"
      :role="stage === 'blocked' ? 'alert' : 'status'"
      aria-live="polite"
    >
...
.startup__seal {
  ...
  place-self: center;
  place-items: center;
  border: 1px solid color-mix(in srgb, var(--color-companion) 40%, var(--color-border));
  border-radius: 1.45rem 1.45rem 1.45rem 0.35rem;
...
.startup__dot {
  ...
  box-shadow: 0 0 0.6rem var(--color-companion);
  animation: pulse 1.5s ease-in-out infinite;
}
.startup--blocked .startup__dot {
  background: var(--color-error);
  box-shadow: 0 0 0.6rem var(--color-error);
  animation: none;
}
