...
async function restore(memoryId: string): Promise<void> {
  await recycleBin.restore(memoryId)
}
...
<template>
  <main class="recycle-bin-view">
    <header class="header">
      ...
      <div>
        <h1>回收站</h1>
        <p>这里暂存已整理的记忆，你可以恢复或在确认后清空。</p>
      </div>
      <button type="button" class="empty" :disabled="recycleBin.state.total === 0" @click="empty">
        清空回收站
      </button>
    </header>
    ...
    <p v-else-if="recycleBin.state.items.length === 0" class="empty-copy">
      这里暂时没有需要整理的记忆。
    </p>
    <ul v-else class="list">
      <li v-for="item in recycleBin.state.items" :key="item.id">
        <div>
          <p>{{ item.content }}</p>
          <small>{{ item.type }} · 重要度 {{ item.importance }}</small>
        </div>
