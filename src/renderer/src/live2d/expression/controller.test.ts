...
function makeRenderer(): { renderer: ILive2DRenderer; expressions: string[] } {
  ...
  const result = {
    ...
    setExpression: async (name: string) => { expressions.push(name); return true },
    playMotion: async () => true, setParameter: () => true, setMouthOpen: () => true, setEyeOpen: () => true,
    hitTest: () => [], getMetrics: () => ({ fps: 0, frameCount: 0, modelLoadMs: null, contextLossCount: 0, paused: false, hasModel: true }), dispose() { /* noop */ }
...
describe('P3A-22 ExpressionController', () => {
  it('至少三种情绪可触发表情，缺失情绪回 neutral alias', async () => {
    ...
    const controller = createExpressionController({ renderer, expressionNames: ['normal', 'smile', 'sad'] })
  ...
  it('刷新期限在 179s 不回 neutral，180s 后回 neutral；刷新可延长期限', async () => {
    const { renderer, expressions } = makeRenderer()
    const controller = createExpressionController({ renderer, expressionNames: ['neutral', 'smile'], resetAfterMs: 180_000 })
    await controller.setEmotion('smile')
  ...
  it('默认静默 15 秒后回 neutral：14.9s 仍保持，15s 到点复位', async () => {
    ...
    const controller = createExpressionController({ renderer, expressionNames: ['neutral', 'smile'] })
