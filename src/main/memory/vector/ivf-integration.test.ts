...
describe('V-03b：IVF 构建窗口期增删的重放（2026-08-20 修复）', () => {
  let t: TestDb
  beforeEach(async () => {
    t = await makeMemoryDb()
  })
  ...
  // V-03b 修复（修复清单第四部分）：
  //   原问题：窗口期 upsert 被 addToIvf 跳过、新索引由窗口前快照构建 -> 检索盲区。
  //   修法：窗口期 upsert/remove 暂存 pendingDuringBuild，构建结束在 finally 重放。
  // 本测试用闸门 kmeansBuilder 把"构建窗口"变成确定性的：构建开始即挂起，测试手动放行。
  it('窗口期 upsert 的向量与 remove 的条目在构建结束后正确反映到新索引', async () => {
    const dim = 8
    ...
    // 构建窗口期：upsert 目标向量 X（首维远超噪声量级）+ remove 已在快照中的 l2_0
    ins.run('l2_x', 'm', 0.5)
    ...
    // 窗口内检索：ivfBuilding=true → 暴力扫描 → X 必中（证明 X 确实在库且查询有效）
    ...
    // 修复后：窗口期 upsert 的 X 经 pendingDuringBuild 重放补进新索引 -> 可检索
    const afterHits = store.search(x, 5, -1)
    expect(afterHits.some((h) => h.memoryId === 'l2_x')).toBe(true)

    // 窗口期 remove 的 l2_0（含在构建快照中）经重放从新索引移除 -> 不可检索
    const removedHits = store.search(v0 as Float32Array, 1000, -1)
    expect(removedHits.some((h) => h.memoryId === 'l2_0')).toBe(false)
  })
})
