...
function turnTruth(): DmaeTurnRecord {
  return {
    turn: 1,
    ts: 1,
    eligibleActive: 15_000,
    retrievalHits: 15,
    promptSelected: 15,
    maxActive: 15,
    promptIncluded: 12,
    promptTrimmed: 3,
    promptIncludedIds: [id(0), id(2)],
    promptTrimmedIds: [id(1)],
    userHits: 0,
    modelHits: 0,
    modelHitsGated: 0,
    modelRewardRawSum: 0,
...
function service(
...
): ReturnType<typeof createDmaeDiagnosticsService> {
  ...
  const history: DmaeHistoryStore = {
    ...
    querySamples: () => [] as DmaeSamplePoint[],
    queryAllSamples: () => [],
    queryRecentSamples: () => [],
    queryTurns: () => turns,
  ...
  const dmae: DmaeEngineService = {
    ...
    getActivation: () => 0,
    getStats: () => ({ active: total, dormant: 0, archived: 0 }),
    getL2Total: () => total,
  ...
  const l2: Pick<L2Store, 'count' | 'list' | 'get'> = {
    ...
    get: (memoryId) => records.find((record) => record.id === memoryId) as never
  }
...
describe('P3X DMAE panel truth and paging', () => {
  it('separates selectL2 candidates from final PromptBudgeter injection truth', () => {
    ...
    expect(snapshot.selection).toMatchObject({
      lastPromptSelectedCount: 15,
      lastPromptIncludedCount: 12,
      lastPromptTrimmedCount: 3
    })
  ...
  it('returns bounded 100-entry pages with a stable keyset cursor at 15k scale', () => {
    ...
    const second = diagnostics.getPanelSnapshot({
      eligibleLimit: 100,
      eligibleCursor: first.nextEligibleCursor!
    })
    expect(second.activeSet).toHaveLength(100)
    expect(
      new Set([...first.activeSet, ...second.activeSet].map((entry) => entry.memoryId)).size
    ).toBe(200)
  })

  it('marks stale-turn cursors as reset instead of appending an unstable page', () => {
    const snapshot = service(15_000).getPanelSnapshot({
      eligibleCursor: { turn: 0, activation: 100, memoryId: id(0) },
      eligibleLimit: 10
    })
