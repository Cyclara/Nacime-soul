...
describe('P3C1-08 persistence：recordTurn', () => {
  it('写入全列（含时序遥测两列），默认审计列为 NULL', () => {
    ...
    const row = db
      .prepare(`SELECT * FROM compliance_turns WHERE turn_id = 't-1'`)
  ...
  it('时序遥测两列可为 NULL（provider 零 delta 的空回复轮）', () => {
    ...
    const row = db
      .prepare(`SELECT provider_first_delta_ms, gate_hold_ms FROM compliance_turns WHERE turn_id = 't-empty'`)
      .get() as Record<string, unknown>
  ...
  it('retryTurn 复用 turnId -> INSERT OR REPLACE（同轮只留最新终局）', () => {
    ...
    const count = (db.prepare(`SELECT COUNT(*) AS n FROM compliance_turns`).get() as { n: number }).n
    expect(count).toBe(1)
    const row = db
  ...
  it('关闭并重开文件数据库后 samples 计数仍存在（C1 验收⑤：跨重启不归零）', () => {
    ...
    dbs.push(restarted)
    const count = (
      restarted.prepare(`SELECT COUNT(*) AS n FROM compliance_samples WHERE turn_id = 't-restart'`).get() as {
        n: number
      }
    ).n
    expect(count).toBe(1)
...
describe('P3C1-08 persistence：recordSamples', () => {
  ...
  it('wouldBlock=false 时 block_ineligible_reason 落枚举值', () => {
    const row = sampleRowFromRecord(
      record({ wouldBlockUnderFirstSegmentPolicy: false, blockIneligibleReason: 'after-first-segment' }),
      'meta-reference',
      'critical',
      7777
    )
  ...
  it('批写多行进库；空数组 no-op', () => {
    ...
    const count = (db.prepare(`SELECT COUNT(*) AS n FROM compliance_samples`).get() as { n: number }).n
    expect(count).toBe(2)
    const ruleIds = (
  ...
  it('表无 content 列（红线守卫，迁回 009 validate）', () => {
    const db = makeDb()
    const cols = (db.prepare(`PRAGMA table_info(compliance_samples)`).all() as { name: string }[]).map(
      (c) => c.name
    )
...
describe('P3C1-08 persistence：recordAuditResult', () => {
  function makeTurnedDb(): { db: Database.Database; p: ReturnType<typeof createCompliancePersistence> } {
    const db = makeDb()
    const p = createCompliancePersistence({ db })
    p.recordTurn({
      turnId: 't-1',
  ...
  it('完成审计回填四列（audited=1 / verdict / level / unavailable=0）', () => {
    ...
    const row = db
      .prepare(`SELECT audited, audit_verdict, audit_level, audit_unavailable FROM compliance_turns WHERE turn_id = 't-1'`)
      .get() as Record<string, unknown>
  ...
  it('unavailable 空壳也入账（§3.11：audit_unavailable=1，verdict/level NULL）', () => {
    ...
    const row = db
      .prepare(`SELECT audited, audit_verdict, audit_unavailable FROM compliance_turns WHERE turn_id = 't-1'`)
      .get() as Record<string, unknown>
    expect(row['audited']).toBe(1)
    expect(row['audit_verdict']).toBeNull()
  ...
  it('turns 行不存在（无门控轮/已清理）-> 返回 false，UPDATE 与 llm sample 均 zero-write', () => {
    ...
    const turns = (db.prepare(`SELECT COUNT(*) AS n FROM compliance_turns`).get() as { n: number }).n
    expect(turns).toBe(1) // 只有 t-1，没建新行
    const samples = (
      db.prepare(`SELECT COUNT(*) AS n FROM compliance_samples WHERE turn_id = 't-unknown'`).get() as {
        n: number
      }
    ).n
  ...
  it('pass 无违规 -> 不写 llm 行', () => {
    ...
    const samples = (db.prepare(`SELECT COUNT(*) AS n FROM compliance_samples`).get() as { n: number }).n
    expect(samples).toBe(0)
  })
})
...
describe('P3C1-08 persistence：purgeStale', () => {
  it('超期 turns 级联清理 samples/feedback；期内全保留；孤儿行一并清', () => {
    ...
    p.recordSamples([sampleRowFromRecord(record({ turnId: 't-stale' }), 'meta-reference', 'critical', now - 91 * day)])
    p.recordSamples([sampleRowFromRecord(record({ turnId: 't-fresh' }), 'meta-reference', 'critical', now - 10 * day)])
    db.prepare(
    ...
    const turns = (db.prepare(`SELECT COUNT(*) AS n FROM compliance_turns`).get() as { n: number }).n
    const samples = (db.prepare(`SELECT COUNT(*) AS n FROM compliance_samples`).get() as { n: number }).n
    const feedback = (db.prepare(`SELECT COUNT(*) AS n FROM compliance_feedback`).get() as { n: number }).n
    expect(turns).toBe(1)
