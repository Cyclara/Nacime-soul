...
describe('P3A-23 Live2D handlers', () => {
  ...
  it('get-state 只返回注入 snapshot，select-model 只向 manager 传受控 URL', async () => {
    ...
    const service = {
      getRegistered: () => ({ id: 'mao' }),
      select: () => true,
      getStageModelUrl: () => 'nacime-live2d://model/mao/Mao.model3.json',
      getLoadPlan: () => ({
        attempts: [],
        exhaustedError: {
          code: 'FILE_NOT_FOUND',
    ...
    const manager = {
      ...
      requestModelLoad: vi.fn((modelId: string) => {
        sendStageCommand({
          type: 'load-model',
          modelUrl: `nacime-live2d://model/${modelId}/Mao.model3.json`
        })
        return true
      })
    ...
    await expect(handler('companion:live2d:get-state')(event(), undefined)).resolves.toEqual({
      ok: true,
      data: snapshot
    })
    await expect(
      handler('companion:live2d:select-model')(event(), { modelId: 'mao' })
    ).resolves.toMatchObject({ ok: true })
    expect(sendStageCommand).toHaveBeenCalledWith({
      type: 'load-model',
  ...
  it('select-model 拒绝未知 modelId，带绝对路径/多余字段的载荷在 validator 层拒绝', async () => {
    ...
    registerLive2dHandlers({
      ...
      service,
      importer: {} as Live2dModelImporter,
      manager: {} as Live2dWindowManager,
      getSnapshot: () => {
        throw new Error('should not call')
      }
    })
    const invoke = handler('companion:live2d:select-model')
    expect(await invoke(event(), { modelId: 'C:\\secret.model3.json' })).toMatchObject({
      ok: false,
      error: { code: 'IPC_VALIDATION' }
    })
    expect(await invoke(event(), { modelId: 'mao', path: 'C:\\secret' })).toMatchObject({
      ok: false,
      error: { code: 'IPC_VALIDATION' }
    })
    expect(await invoke(event(), { modelId: 'missing' })).toMatchObject({
      ok: false,
      error: { code: 'L2D_MODEL_LOAD' }
    })
  })
})
